...
const Nav = () => {
  ...
  return (
    <nav className="flex-between w-full mb-16 pt-3">
      ...
      <p className="logo_text">TechTalk</p>
      </Link>
      ...
      <div className="sm:flex hidden">
        ...
        ? (<div className="flex gap-3 md:gap-5">
          ...
          <Link href="/profile">
          <Image src={session?.user.image} width={30} height={30} alt="Profile Icon"/>
          </Link>
        </div>) 
        : (<>
         {providers && Object.values(providers).map((provider)=>(
          <button type="button" key={provider.id} className="black_btn" onClick={()=> signIn(provider.id)}>Sign In</button>
         ))}
        </>)}
      </div>
